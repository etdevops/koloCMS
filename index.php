...
if (!empty($_POST['username']) && !empty($_POST['password'])) { // checking if it's a post req
    $sha1pass = lookup($_POST['username']);
		
    if ($sha1pass == sha1($_POST['password'])) { // checking the password and setting session
	$_SESSION['login'] = true;
...
<html>
	...
	<form method="POST" action="/">
		...
		<?php if (isset($fail)) { echo '<div class="w3-panel w3-card-4 w3-bottombar w3-monospace" style="color:red; font-weight:12px; background-color: black; font-size:20px;" ><a class=""></a><p>Invalid username or password</p></div>'; } ?>
	  </div>
